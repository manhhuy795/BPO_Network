...
FILE_LOG="$THU_MUC_GOC/logs/phase_e_test.log"
cd "$THU_MUC_GOC"
mkdir -p logs
...
python3 topology/topology_v1.py --kiem-thu-e 2>&1 | tee "$FILE_LOG"
ma_loi=${PIPESTATUS[0]}

if [[ "$ma_loi" -eq 0 ]]; then
    printf '[ĐẠT] Toàn bộ kiểm thử Giai đoạn E đã đạt.\n' | tee -a "$FILE_LOG"
else
    printf '[KHÔNG ĐẠT] Giai đoạn E còn lỗi, mã thoát %s.\n' "$ma_loi" | tee -a "$FILE_LOG"
fi
