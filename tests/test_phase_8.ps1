...
$SqlFile = Join-Path $PSScriptRoot "test_phase_8.sql"
$TestDatabase = "bpo_phase8_" + [guid]::NewGuid().ToString("N")
...
try {
    ...
    & docker cp $SqlFile "bpo-postgres:/tmp/test_phase_8.sql" *> $null
    & docker exec -e "PGPASSWORD=$Password" bpo-postgres `
        psql -v ON_ERROR_STOP=1 -U $User -d $TestDatabase -f /tmp/test_phase_8.sql
    if ($LASTEXITCODE -ne 0) { throw "Ground truth Phase 8 khong dat." }

    Write-Host "[DAT] Phase 8: topology correlation, fallback, duplicate va firing/resolved deu dat."
    exit 0
} catch {
    Write-Host "[KHONG DAT] Phase 8 - $($_.Exception.Message)"
    exit 1
} finally {
    & docker exec -e "PGPASSWORD=$Password" bpo-postgres `
        psql -U $User -d $Database -c "DROP DATABASE IF EXISTS $TestDatabase WITH (FORCE);" *> $null
    & docker exec bpo-postgres rm -f /tmp/test_phase_8.sql *> $null
}
