...
FILE_LOG="$THU_MUC_GOC/logs/phase_g_test.log"
cd "$THU_MUC_GOC"
mkdir -p logs runtime docs
...
python3 topology/topology_v2_dual_wan.py --kiem-thu-g 2>&1 | tee "$FILE_LOG"
ma_loi=${PIPESTATUS[0]}

if [[ "$ma_loi" -eq 0 ]]; then
    printf '[ĐẠT] Toàn bộ phép đo Giai đoạn G đã đạt.\n' | tee -a "$FILE_LOG"
else
    printf '[KHÔNG ĐẠT] Giai đoạn G còn lỗi, mã thoát %s.\n' "$ma_loi" | tee -a "$FILE_LOG"
fi
